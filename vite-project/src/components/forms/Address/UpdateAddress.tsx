...
const UpdateAddress: React.FC<CreateAddressProps> = ({setOpen, setSuccess}) => {
    ...
    const handleRemoveAddress = async (id: string) => {
        ...
        deleteData(endpoints.user.deleteAddress, payload)
    }
            ...
            <Formik initialValues={{
                ...
                is_default: false,
                    ...
                    onSubmit={async (values) => {
                        try {
                            const response = await apiClient.put(endpoints.user.updateAddress, values);
                ...
                {({
                    ...
                    return (
                        <Form className={'w-full h-full p-2 flex flex-col justify-start items-center'}
                            ...
                            <div className="w-full h-fit grid grid-cols-3 grid-rows-1 gap-4 my-3">
                                <button type="button"
                                        className="border-2 border-[rgb(var(--border-color))] p-2 rounded-lg"
                                        onClick={() => setOpen(false)}>
                                    Hủy
                                </button>
                                <button type="button"
                                        className="border-2 border-[rgb(var(--border-color))] p-2 rounded-lg bg-red-500 text-white"
                                        onClick={() => handleRemoveAddress(values.id)}>
                                    Xóa
                                </button>
                                <button type="submit"
                                        className="border-2 border-[rgb(var(--border-color))] p-2 rounded-lg bg-[rgb(var(--btn-primary-bg))] text-[rgb(var(--btn-primary-text))]"
                                >
                                    Xác nhận
                                </button>
                            </div>
